...
PASTA_SETTINGS = Path(__file__).parent / 'settings'
PASTA_SETTINGS.mkdir(exist_ok=True)
...
def salva_chave(chave):
    with open(PASTA_SETTINGS / 'chave', 'wb') as f:
        pickle.dump(chave, f)

def le_chave():
    if (PASTA_SETTINGS / 'chave').exists():
        with open(PASTA_SETTINGS / 'chave', 'rb') as f:
